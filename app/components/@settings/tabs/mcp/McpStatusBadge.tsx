  ...
  const { styles, label, icon, ariaLabel } = useMemo(() => {
    ...
    const config = {
      checking: {
        ...
        icon: <span className="i-svg-spinners:90-ring-with-bg h-3 w-3 animate-spin text-current" aria-hidden="true" />,
      },
      available: {
        styles: `${base} bg-green-100 text-green-800 dark:bg-green-900/80 dark:text-green-200`,
        label: 'Available',
        ariaLabel: 'Server available',
        icon: <span className="i-ph:check-circle h-3 w-3 text-current" aria-hidden="true" />,
      },
      unavailable: {
        styles: `${base} bg-red-100 text-red-800 dark:bg-red-900/80 dark:text-red-200`,
        label: 'Unavailable',
        ariaLabel: 'Server unavailable',
        icon: <span className="i-ph:warning-circle h-3 w-3 text-current" aria-hidden="true" />,
      },
    };
